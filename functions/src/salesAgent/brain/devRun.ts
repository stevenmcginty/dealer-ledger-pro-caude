...
import { rankStock } from '../stock/search';
import type { StockApi } from './tools';
import type { AgentMessage, Conversation, InboundMessage, SalesAgentSettings, StockItem } from '../types';
 ...
 */
const stubStock: StockApi = {
    async searchStock(_companyId, filters) {
        const hits = rankStock(STOCK, filters);
...
const SETTINGS: SalesAgentSettings = {
    ...
    phone: '07710 525694',
    faqs:
        'Warranty: 3 months minimum on every car, extendable to 12 or 24 months. ' +
...
const main = async (): Promise<void> => {
    ...
    for (let turn = 0; turn < SCRIPT.length; turn++) {
        ...
        const result = await runBrain(
            { companyId: conversation.companyId, conversation, history, inbound, settings: SETTINGS },
            { stock: stubStock, apiKey },
        );
