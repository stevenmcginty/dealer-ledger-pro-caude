...
const parseCarGurusLead = (raw: RawEmail): ParsedLead => {
    ...
    const vehicle = {
        ...
        reg: findReg(text.match(/\bReg:\s*([A-Z0-9 ]{2,10})/i)?.[1] || starField(text, 'Reg')),
...
const parseCarDealer5Enquiry = (raw: RawEmail): ParsedLead => {
    ...
    const reg = findReg(raw.subject.match(/\(([A-Z0-9\s]{2,10})\)/i)?.[1], raw.subject);
...
export const parseLeadEmail = (input: RawEmail): ParsedLead => {
    ...
    const raw: RawEmail = input.text?.trim() ? input : { ...input, text: htmlToText(input.html) };
    const sender = parseFromHeader(raw.from);
