...
describe('a follow-up that names a different car', () => {
    ...
    it('"would it be possible to view the MX5 today" is the MX-5 for sale, not the Astra', () => {
        const text = 'Hi Chris / Steve, Hope all is well! Would it be possible to view the MX5 today? Thanks';
        assert.equal(matchEnquiryStock(STOCK, { text })?.id, 'mx5-live');
        assert.deepEqual(rankStock(STOCK, { text, includeHidden: true, limit: 5 }).filter(h => h.matchQuality !== 'weak').map(h => h.id), ['mx5-live']);
    });

    it('a chatty message with no car in it names nothing', () => {
        for (const text of ['Can I come and see it tomorrow?', 'Is the roof operating ok?', 'Hope all is well!']) {
            assert.equal(matchEnquiryStock(STOCK, { text }), null, text);
        }
    });
});
