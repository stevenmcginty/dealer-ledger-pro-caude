...
exports.GEMINI_SECRET_NAME = 'GEMINI_API_KEY';
exports.BRAIN_MODEL = 'gemini-2.5-flash';
/** How many batches of tool calls we will service before forcing an answer. */
exports.MAX_TOOL_ROUNDS = 4;
exports.MAX_REPLY_SENTENCES = 3;
exports.MAX_REPLY_CHARS = 400;
exports.MAX_EMAIL_REPLY_SENTENCES = 8;
exports.MAX_EMAIL_REPLY_CHARS = 1200;
// ---------------------------------------------------------------------------
// Guards
// ---------------------------------------------------------------------------
/**
 ...
 */
const softenDashes = (text) => text
    .replace(/\s*[—―]\s*/g, ', ')
    .replace(/\s+–\s+/g, ', ')
    .replace(/\s*,\s*,\s*/g, ', ')
    .replace(/\s*,\s*([.!?])/g, '$1')
    .replace(/,\s*$/, '');
exports.softenDashes = softenDashes;
const capBody = (text, maxSentences = exports.MAX_REPLY_SENTENCES, maxChars = exports.MAX_REPLY_CHARS) => {
...
const capEmailBody = (text, maxSentences = exports.MAX_EMAIL_REPLY_SENTENCES, maxChars = exports.MAX_EMAIL_REPLY_CHARS) => {
    const rawParagraphs = text.split(/\n\s*\n/).map(p => p.trim()).filter(Boolean);
    if (!rawParagraphs.length)
        return capBody(text, maxSentences, maxChars);
    const outParagraphs = [];
    let usedSentences = 0;
    let currentTotalLength = 0;
    for (const para of rawParagraphs) {
        if (usedSentences >= maxSentences || currentTotalLength >= maxChars)
            break;
        const cleanPara = para.replace(/\s+/g, ' ').trim();
        const sentences = cleanPara.match(/[^.!?]*[.!?]+["')\]]*\s*|[^.!?]+$/g) || [cleanPara];
        let paraOut = '';
        for (const sentence of sentences) {
            if (usedSentences >= maxSentences)
                break;
            if (currentTotalLength + paraOut.length + sentence.length > maxChars)
                break;
            paraOut += sentence;
            usedSentences++;
        }
 ...
 */
const capReply = (text, channel = 'whatsapp') => {
    const raw = text.replace(/\r\n/g, '\n').trim();
    if (!raw)
        return '';
    if (channel !== 'email')
        return capBody(raw, exports.MAX_REPLY_SENTENCES, exports.MAX_REPLY_CHARS);
    const lines = raw.split('\n');
    let greeting = '';
    if (lines.length > 1 && GREETING_LINE.test(lines[0].trim()))
    ...
    const body = capEmailBody(remaining, exports.MAX_EMAIL_REPLY_SENTENCES, exports.MAX_EMAIL_REPLY_CHARS);
 ...
 */
const listedPriceFor = (effects, conversation) => {
    const focus = effects.vehicleInterest?.stockId || conversation.vehicleInterest?.stockId;
    if (focus && effects.seenPrices.has(focus))
        return effects.seenPrices.get(focus);
...
exports.scanFigures = scanFigures;
/**
 ...
 */
const unvouchedFigure = (reply, args) => {
...
exports.unvouchedFigure = unvouchedFigure;
/**
 ...
 */
const applyPriceGuard = (args) => {
    const { reply, settings, listedPrice, quoted, inboundText } = args;
    if (settings.priceFlexMode !== 'hint' || !reply)
        return { reply };
    const bad = (0, exports.unvouchedFigure)(reply, { listedPrice, quoted });
    if (!bad)
        return { reply };
    return {
...
exports.applyPriceGuard = applyPriceGuard;
const inferStage = (known, current) => {
    if (current === 'closed')
        return 'closed';
    if (known.booking)
        return 'booked';
    if (known.preferredTime)
        return 'details';
    if (known.partExOrFinance)
        return 'timing';
    if (known.vehicleInterest && (known.vehicleInterest.stockId || known.vehicleInterest.title))
        return 'deal';
    return 'vehicle';
};
...
const deriveSummary = (conversation, inbound) => {
    const bits = [];
    ...
    if (conversation.vehicleInterest?.title)
        bits.push(`Interested in the ${conversation.vehicleInterest.title}.`);
    if (conversation.partExOrFinance)
        bits.push(`Part-ex or finance: ${conversation.partExOrFinance}.`);
    if (conversation.preferredTime)
        bits.push(`Suggested ${conversation.preferredTime} for a viewing.`);
    if (conversation.booking)
        bits.push(`Viewing logged for ${conversation.booking.window}.`);
    const last = (inbound.text || '').replace(/\s+/g, ' ').trim().slice(0, 140);
    if (last)
        bits.push(`Last message: "${last}".`);
    return bits.join(' ');
};
// ---------------------------------------------------------------------------
// The loop
...
const runBrain = async (input, deps = {}) => {
    ...
    if (!apiKey)
        throw new Error(`${exports.GEMINI_SECRET_NAME} is not set; the sales agent cannot run`);
    const ai = new genai_1.GoogleGenAI({ apiKey });
    const effects = (0, tools_1.newToolEffects)();
    const ctx = {
        companyId,
        conversation,
        settings,
    ...
    const configWithoutTools = { ...config };
    delete configWithoutTools.tools;
    const usage = { inputTokens: 0, outputTokens: 0, model: exports.BRAIN_MODEL };
    ...
    for (let round = 0; round < exports.MAX_TOOL_ROUNDS; round++) {
        ...
        for (const call of calls) {
            const result = await (0, tools_1.runTool)(call.name || '', call.args || {}, ctx);
    ...
    let reply = (0, exports.softenDashes)(asString(parsed.reply) || '');
    let escalate = effects.escalate;
    const businessFigures = (0, exports.scanFigures)(effects.businessText);
    const guarded = (0, exports.applyPriceGuard)({
        reply,
        settings,
    ...
    reply = (0, exports.capReply)(guarded.reply, conversation.channel);
    if (guarded.escalate)
    ...
    if (preferredTime && preferredTime !== conversation.preferredTime)
        updates.preferredTime = preferredTime;
    if (effects.booking)
        updates.booking = effects.booking;
    ...
    if (JSON.stringify(contact) !== JSON.stringify(conversation.contact || {}))
    ...
    const result = { reply, stage, updates, usage };
    if (escalate)
        result.escalate = escalate;
    if (effects.askOwner)
        result.askOwner = effects.askOwner;
    if (effects.handoff)
        result.handoff = true;
    return result;
};
...
Object.defineProperty(exports, "toolDeclarations", { enumerable: true, get: function () { return tools_2.toolDeclarations; } });
