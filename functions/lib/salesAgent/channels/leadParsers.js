...
exports.isNoReplyAddress = isNoReplyAddress;
...
const bodyEmails = (text, selfEmail) => {
    const found = (text || '').match(EMAIL_RE) || [];
    const self = (selfEmail || '').toLowerCase();
    return Array.from(new Set(found.map(e => e.toLowerCase())))
        .filter(e => e !== self && !(0, exports.isNoReplyAddress)(e));
};
const firstPhone = (...sources) => {
    for (const source of sources) {
        const found = (0, types_1.extractUkMobiles)(source || '');
        if (found.length)
            return found[0];
    }
    return undefined;
 ...
 */
const buildReplyTargets = (email, phone) => {
    const targets = [];
    if (email && !(0, exports.isNoReplyAddress)(email))
        targets.push({ channel: 'email', address: email.toLowerCase() });
    if (phone)
        targets.push({ channel: 'whatsapp', address: (0, types_1.toE164)(phone) }, { channel: 'sms', address: (0, types_1.toE164)(phone) });
    return targets;
};
const withReply = (lead) => {
 ...
 */
const looksLikeSpam = (text, html, selfEmail) => {
    const body = `${text || ''}\n${html || ''}`;
    const offPlatform = linkDomains(body).filter(host => !TRUSTED_LINK_DOMAINS.some(d => host === d || host.endsWith(`.${d}`)));
    if (!offPlatform.length)
        return false;
    const hasPlainContact = bodyEmails(text || '', selfEmail).length > 0 || (0, types_1.extractUkMobiles)(text || '').length > 0;
...
const parseCarGurusPhoneLead = (raw) => {
    const text = raw.text || '';
...
const cazooFlags = (lines, enquiryType) => {
    const haystack = `${lines.join(' ')} ${enquiryType || ''}`;
    const flags = {};
    if (/test\s*drive/i.test(haystack))
        flags.testDrive = true;
    if (/service\s*history/i.test(haystack))
        flags.wantsServiceHistory = true;
    if (/photo/i.test(haystack))
        flags.wantsMorePhotos = true;
    if (/video/i.test(haystack))
        flags.wantsVideo = true;
 ...
 */
const blockAfter = (text, heading, stops) => {
    ...
    for (let i = start + 1; i < lines.length; i++) {
        ...
        if (stops.some(stop => stop.test(line)))
            break;
        if (line)
            out.push(clean(line));
    }
...
const parseCazooEnquiry = (raw) => {
    ...
    if (!message && flags?.testDrive)
        message = `Wants a test drive of the ${title || 'car'}`;
    return withReply({
        source: 'Cazoo',
        kind: 'enquiry',
...
const parseGumtreeMissedCall = (raw) => {
    const text = raw.text || '';
    const phone = firstPhone(text.match(/reach the customer on\s*([+0-9][0-9\s]{6,})/i)?.[1], text.match(/Call back\s*([+0-9][0-9\s]{6,})/i)?.[1], text);
    return withReply({
        source: 'Cazoo',
        kind: 'missed_call',
        phone,
...
const parseCarDealer5Enquiry = (raw) => {
    ...
    $('table.personal-info tr').each((_i, row) => {
        ...
        if (label)
            fields[label] = { text: clean($(cells[1]).text()), html: $(cells[1]) };
    });
...
const parseDirectEmail = (raw, source) => {
    ...
    return withReply({
        ...
        vehicle: { reg: (0, exports.findReg)(subject, text) },
        message: clean(text),
...
const parseLeadEmail = (raw) => {
    ...
    if ((0, exports.isCazooReservationPhish)(subject, text)) {
        return ignored('Other', 'phishing:cazoo_reservation_request');
    }
    if ((0, exports.looksLikeSpam)(text, raw.html, raw.selfEmail)) {
        return ignored('Other', 'spam:off_platform_link_no_contact');
    }
    const domain = from.split('@')[1] || '';
    if (/ebay\.co\.uk$/i.test(domain))
...
const isCazooReservationPhish = (subject, text) => {
    if (/reservation request from cazoo/i.test(subject || ''))
        return true;
    const hits = RESERVATION_CHECKBOXES.filter(re => re.test(text || '')).length;
    return hits >= 3 && /\bCustomer\s*:/i.test(text || '');
};
exports.isCazooReservationPhish = isCazooReservationPhish;
/** ParsedLead sources are platform names; the CRM has its own shorter list. */
const crmLeadSource = (source) => {
    switch (source) {
