 ...
 */
const INVOICE_FAMILY = [
    {
        name: 'invoice_notice',
        params: ([name, doc]) => [name || 'there', doc || 'invoice'],
        render: ([name, doc]) => `Hi ${name || 'there'}, your ${doc || 'invoice'} from Radlett Cars is in an email we have just sent you. Please check your inbox and spam folder. Reply here if it has not arrived.`,
    },
];
/**
 ...
 */
exports.OWNER_MESSAGE_TEMPLATE = 'customer_update';
exports.OWNER_MESSAGE_PLAIN_TEMPLATE = 'owner_message';
/** Meta caps a rendered body at 1024 characters; the wrapper takes ~120 of them. */
exports.OWNER_MESSAGE_CHUNK = 800;
const OWNER_MESSAGE_FAMILY = [
    {
        name: exports.OWNER_MESSAGE_TEMPLATE,
        params: ([name, words]) => [name || 'there', words || '-'],
        render: ([name, words]) => `Hi ${name || 'there'}, this is Radlett Cars with an update about your car. ${words || '-'} Reply to this message if you have any questions.`,
    },
    {
        name: exports.OWNER_MESSAGE_PLAIN_TEMPLATE,
        params: ([name, words]) => [name || 'there', words || '-'],
        render: ([name, words]) => `Hi ${name || 'there'}, this is Radlett Cars. ${words || '-'} You can reply to this message.`,
    },
];
/**
 ...
 */
const splitForTemplate = (text, max = exports.OWNER_MESSAGE_CHUNK) => {
...
exports.splitForTemplate = splitForTemplate;
/** Has Meta approved any member of this template's family right now? */
const templateFamilyApproved = async (companyId, templateName) => {
...
exports.templateFamilyApproved = templateFamilyApproved;
