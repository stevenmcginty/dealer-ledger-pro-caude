...
exports.handleOwnerCommand = void 0;
...
const resolve = async (companyId, shortId) => {
    const n = Number(String(shortId).replace('#', ''));
    if (!Number.isFinite(n))
        return null;
    const convId = await (0, conversations_1.convIdForShortId)(companyId, n);
    if (!convId)
        return null;
    return (0, conversations_1.getConversation)(companyId, convId);
};
const setEnabled = async (companyId, enabled) => {
    await (0, conversations_1.db)().ref((0, conversations_1.agentPath)(companyId, 'settings')).update({ enabled, updatedAt: Date.now() });
};
/** Send something to the customer as Steve, right now. */
const replyAsOwner = async (companyId, conversation, text) => {
    const job = {
        id: 'owner-reply',
        companyId,
    ...
    await (0, outbox_1.sendNow)(job, 'owner');
};
/**
 ...
 */
const handleOwnerCommand = async (companyId, settings, raw) => {
    const text = (raw || '').trim();
    if (!text)
        return;
    try {
        ...
        if (upper === 'PAUSE ALL') {
            ...
            await (0, alerts_1.sendOwnerText)(companyId, 'Agent paused. Nothing will be answered until you send RESUME ALL.');
            return;
        }
        if (upper === 'RESUME ALL') {
            await setEnabled(companyId, true);
            await (0, alerts_1.sendOwnerText)(companyId, 'Agent back on.');
            return;
        }
        if (upper === 'STOCK') {
            await (0, alerts_1.sendOwnerText)(companyId, 'Re-indexing stock now, back in a couple of minutes.');
            try {
                const meta = await (0, stock_1.indexStock)(companyId);
                await (0, alerts_1.sendOwnerText)(companyId, `Stock index done: ${meta.availableCount} available of ${meta.count}${meta.errors.length ? ` (${meta.errors.length} errors)` : ''}.`);
            }
            catch (e) {
                await (0, alerts_1.sendOwnerText)(companyId, `Stock re-index failed: ${e.message}`);
            }
            return;
        }
        if (upper === 'STATUS') {
            ...
            await (0, alerts_1.sendOwnerText)(companyId, conversations.length ? `${header}\n${conversations.map(describe).join('\n')}` : header);
            return;
        }
        ...
        if (takeOver) {
            ...
            if (!conversation)
                return (0, alerts_1.sendOwnerText)(companyId, `No conversation #${takeOver[1]}.`);
            ...
            await (0, conversations_1.updateConversation)(companyId, conversation.id, { mode: 'human', pendingDraft: null });
            await (0, alerts_1.sendOwnerText)(companyId, `#${conversation.shortId} is yours. The agent has gone quiet on it.` +
        ...
        if (resume) {
            ...
            if (!conversation)
                return (0, alerts_1.sendOwnerText)(companyId, `No conversation #${resume[1]}.`);
            await (0, conversations_1.updateConversation)(companyId, conversation.id, { mode: 'agent' });
            await (0, alerts_1.sendOwnerText)(companyId, `#${conversation.shortId} handed back to the agent.`);
            return;
        }
        ...
        if (reply) {
            ...
            if (!conversation)
                return (0, alerts_1.sendOwnerText)(companyId, `No conversation #${reply[1]}.`);
            try {
                ...
                await (0, conversations_1.updateConversation)(companyId, conversation.id, { mode: 'human' });
                await (0, alerts_1.sendOwnerText)(companyId, `Sent to #${conversation.shortId}. It is in your hands now — RESUME ${conversation.shortId} to give it back.`);
            }
            catch (error) {
                await (0, alerts_1.sendOwnerText)(companyId, `Could not send to #${conversation.shortId}: ${error?.message || 'unknown error'}`);
            }
            return;
        }
        ...
        if (tell) {
            ...
            if (!conversation)
                return (0, alerts_1.sendOwnerText)(companyId, `No conversation #${tell[1]}.`);
        ...
        await (0, alerts_1.sendOwnerText)(companyId, `That did not work: ${error?.message || 'unknown error'}`);
    }
};
...
//# sourceMappingURL=ownerCommands.js.map