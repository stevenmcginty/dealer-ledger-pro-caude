...
## Notifications
Every owner alert (new / escalation / question / booking / error) also goes as a Firebase Cloud Messaging web-push to every registered device of the company (`companies/{cid}/salesAgent/pushTokens/{token}`), title "Dave", body = alert text, click opens the conversation in the Agent Inbox. Works on the installed PWA (Android home screen, desktop Chrome). WhatsApp alert remains the fallback/second channel. Optional later: TWA APK wrapper for the Play Store.

## Brain rules
System prompt structure:
1. Role + business: the agent is **Dave** (settings.agentName), on the sales desk at Radlett Car Sales with Steve and Chris (settings.teamNames). Signs as Dave. Completely human, down-to-earth UK sales tone; never volunteers that it is AI; if asked outright, deflects lightly without claiming to be human ("I'm Dave on the sales desk here, Steve or Chris can jump in any time if you'd prefer") and escalates.
2. Channel-specific voice:
   - **WhatsApp/SMS**: concise, relaxed, conversational UK; 1–3 short sentences max; no "I'd be happy to help", no exclamation overload, no em-dashes, no repeated greetings in ongoing chats.
   - **Email**: structured, professional, thorough (up to 8 sentences / 1200 chars across 2–3 clean paragraphs); warm salutation ("Hi <Name>,"), direct answers to all questions, low-pressure viewing/PX call-to-action, and polite sign-off ("Regards,\nDave\nRadlett Car Sales").
...
4. Qualification flow: Vehicle (skip if named) → Deal (PX / finance) → Timing (mornings vs afternoons) → Details (name + phone) → Booking closure.
5. Deeper vehicle knowledge: tool results provide full spec facts (expanded to 30 features and 1500 chars description from Car Dealer 5). State service history clearly (full history / book stamps / cambelt / clutch / 2 keys), MOT expiry, warranty package, and ULEZ compliance (petrols 2006+, diesels late 2015+). If unknown, say checking the book / forecourt car + ask_owner.
6. Smart part-exchange probing: when customer mentions a trade-in, probe for registration and approximate mileage FIRST before alerting Steve. Only call `ask_owner` once both reg and mileage are provided.
7. FAQs from settings (hours, address, warranty, finance partners, test-drive licence rule, delivery).
8. Booking closure: capture full name + phone + window, then `ask_owner` (see Ask-Steve loop) → once Steve confirms, `book_viewing` → "I've logged that with the sales team, one of us will confirm the slot and have the keys ready for you." → owner alert.
...
The agent must NOT confirm a viewing/test-drive time on its own. When a customer proposes or agrees a time, the brain calls `ask_owner` → BrainResult.askOwner. Router: sets `pendingQuestion`, ALWAYS sends a holding line immediately ("Would you mind bearing with me a moment? Let me find that out for you") — the router injects it if the brain returns an empty reply, sends owner alert kind 'question': "#12 ASK: Barry wants to view the Focus ST Saturday 10am. Are we around? Reply `ANSWER 12 <text>`". Steve replies `ANSWER 12 yes, tell him to ask for me` (a plain non-command owner reply while exactly one question is pending counts as the ANSWER) → router stores `ownerAnswer`, clears `pendingQuestion`, runs the brain with the answer injected so the agent relays it naturally, then `book_viewing`. Same tool for anything the agent can't answer from stock/FAQs (part-ex valuations, damage/Cat S, "would you take £X", CarPlay etc.) instead of guessing.

## Instructing Dave (Steve, 26 Aug)
At any point the owner can tell Dave what to communicate — in the Agent Inbox ("Tell Dave what to say") or on WhatsApp `TELL <n> <text>` — and Dave rephrases it in his own voice and carries on the qualification flow. Same mechanism as ANSWER (ownerAnswer injection) but does not require a pending question. Instructions show in the thread as a grey "You told Dave: …" line and are never fed to the model as customer turns.

## Owner control (messages from ownerAlertNumber on WhatsApp are commands)
`TAKE OVER <n>` · `RESUME <n>` · `REPLY <n> <text>` (sent to customer as Steve, stays in human mode) · `ANSWER <n> <text>` (answers Dave's question) · `TELL <n> <text>` (instruct Dave; he rephrases) · `SEND <n>` (approve the drafted email; aliases `APPROVE <n>`, `OK <n>`) · `STATUS` · `PAUSE ALL` · `RESUME ALL` · `STOCK` (re-index now). Unknown text → help message.

## Draft & Approve (Steve, 26 Aug)
Email replies are written but not sent while `settings.emailApprovalMode` is on (the default; undefined counts as on). Instead of queuing, the router stores `conversation.pendingDraft` and alerts the owner with kind 'draft': `#14 Dave drafted reply to Barry (Ford Focus ST-3): "..." Reply: SEND 14 to approve, TELL 14 <changes>, or TAKE OVER 14`. `SEND 14` queues it with no delay and clears the draft; `TELL 14 <changes>` runs the usual instruction path and the rewrite comes back as a new draft rather than going out; `TAKE OVER 14` throws it away. A bare owner reply counts as `TELL` when exactly one conversation has a draft and none has a pending question. The same holds for the ANSWER path: an answer that produces an email reply becomes a draft too. In the app, the Agent Inbox shows the draft in an editable box — approve (edits included), ask Dave to change it, or discard — over `salesAgentApproveDraft` / `salesAgentDiscardDraft`. WhatsApp and SMS are untouched: they answer on their own, because an enquiry answered an hour late is an enquiry lost.
