...
`TAKE OVER <n>` · `RESUME <n>` · `REPLY <n> <text>` (sent to customer as Steve, stays in human mode) · `ANSWER <n> <text>` (answers Dave's question) · `TELL <n> <text>` (instruct Dave; he rephrases) · `STATUS` · `PAUSE ALL` · `RESUME ALL` · `STOCK` (re-index now). Unknown text → help message.
